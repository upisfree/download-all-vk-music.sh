...
for (( i = 0; i < $count; i++ ))
do
  artist=$(cat $1 | jq ".response.items[$i].artist" | cut -c 2- | rev | cut -c 2- | rev | sed -e 's/[\/\\:\*\?"\<\>\|\x00\t]/_/g') # берём файл, вычленяем нужное поле, режем первую кавычку (первый символ), режем вторую кавычку (второй символ), убираем запрещённые символы
  title=$(cat $1 | jq ".response.items[$i].title" | cut -c 2- | rev | cut -c 2- | rev | sed -e 's/[\/\\:\*\?"\<\>\|\x00\t]/_/g')
